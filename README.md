...
Our analysis reveals that Biased SVD achieves a statistically significant RMSE improvement over Item-Item KNN (~9.5%), while the bias-only baseline (BaselineOnly) remains competitive overall (RMSE 0.9229 vs. SVD 0.9300). The SVD advantage over KNN is consistent across all user activity levels, with the largest relative gain observed for the densest users (Q4: 10.87% vs. Q1: 8.82%), highlighting that latent factor models benefit from richer rating histories when benchmarked against neighborhood methods.
...
- A **500,000-rating simple random sample** is used for modeling to balance computational cost with sufficient signal for latent factor learning
...
| scikit-learn | 1.6.1 | PCA for latent factor visualization |
| scipy | 1.16.3 | Statistical significance testing |
...
├── data/
│   └── README.md                  ← Dataset source, download & preprocessing info
└── checkpoints/
...
| **SVD advantage over KNN is consistent across all activity levels** | Table 2: largest gain at Q4/densest (10.87%), smallest at Q2 (5.94%) |
